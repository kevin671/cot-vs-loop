...
import wandb
from experiments.utils import Config, set_seed
from models.coconut import Coconut
from models.looplm import LoopLM
from tasks.prontoqa.prontoqa import (
    MyCollator,
...
def main():
    ...
    if len(cur_ckpts) > 0 and not configs.only_eval:
        ...
        if rank == 0:
            print(f"Warning: found previous run and gonna resume from that. the inputted `resume` argument is ignored!")
    ...
    if configs.load_model_path != "None":
        ...
        elif configs.coconut and any([k.startswith("base_causallm") for k in saved_weights.keys()]):
    ...
    if getattr(configs, "coconut", False):
        model = Coconut(model, latent_id, start_id, end_id, tokenizer.eos_token_id)
    elif getattr(configs, "looplm", False):
        n_loop = getattr(configs, "n_loop", 1)
        model = LoopLM(model, n_loop=n_loop)
    ...
    for epoch in range(configs.resume, configs.num_epochs):
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(valid_gen_dataloader):
                ...
                batch = {k: v.to(rank) for k, v in batch.items() if v != None and k not in ["idx", "position_ids"]}
                ...
                pbar.update(1)
                pbar.set_description(f"Test accuracy: {round(float(cor.detach().float() / total.detach().float()), 2)}")
        ...
        if cor / total > best_acc and configs.save_only_improve and not configs.debug and not configs.only_eval:
            ...
            if rank == 0:
                # torch.save(states, os.path.join(save_dir, f"checkpoint_{epoch + 1}"))
                torch.save(states, os.path.join(save_dir, f"best_checkpoint"))
                print("saving model.")
