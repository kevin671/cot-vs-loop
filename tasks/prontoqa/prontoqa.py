...
def get_dataset(path, tokenizer, max_size=1000000000):
    ...
    d = data[0]
    complete = d["question"] + "\n" + "\n".join(d["steps"]) + "\n### " + d["answer"]
    complete_tokenized = tokenizer.encode(complete, add_special_tokens=True) + [tokenizer.eos_token_id]
    assert (
        complete_tokenized
...
class MyCollator:
    ...
    def __call__(self, features, return_tensors=None):
        ...
        earliest_latent = [
            feature["input_ids"].index(self.latent_id) for feature in features if self.latent_id in feature["input_ids"]
        ]
        ...
        non_label_position_features = [
            {k: v for k, v in feature.items() if k != label_name and k != "position_ids"} for feature in features
        ]
        ...
        if labels is not None and all(label is None for label in labels):
            labels = None
        position_ids = (
            [feature["position_ids"] for feature in features] if "position_ids" in features[0].keys() else None
        )
        ...
        if position_ids is not None:
            ...
            batch["position_ids"] = [
                position_id + [0] * (max_pos_length - len(position_id)) for position_id in position_ids
            ]
...
def get_question_latent_dataset(
    ...
    def process_dataset(sample):
        ...
        if configs.pad_latent_to_max:
            ...
            max_latent_stage = min(configs.max_latent_stage, len(sample["steps_tokenized"]))
    ...
    return base_dataset_valid.map(process_dataset, remove_columns=list(base_dataset_valid.features), num_proc=32)
