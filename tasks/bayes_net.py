import itertools
import random
from collections import defaultdict
from typing import Dict, List

import networkx as nx
import torch
import torch.nn.functional as F
from torch.utils.data import IterableDataset

from tasks.task import GeneralizationTask


def compute_strict_layers(G: nx.DiGraph):
    topo = list(nx.topological_sort(G))
    depth = {node: 0 for node in topo}

    for node in topo:
        parents = list(G.predecessors(node))
        if parents:
            depth[node] = 1 + max(depth[p] for p in parents)

    max_depth = max(depth.values())
    layers = [[] for _ in range(max_depth + 1)]

    for node, d in depth.items():
        layers[d].append(node)

    for u, v in G.edges():
        du, dv = depth[u], depth[v]
        for d in range(du + 1, dv):
            if u not in layers[d]:
                layers[d].append(u)

    layers = [sorted(layer, key=lambda x: topo.index(x)) for layer in layers]
    return layers


def compute_conditional_marginals(G, cpts, root_assignment):
    topo = list(nx.topological_sort(G))
    non_roots = [v for v in topo if v not in root_assignment]

    marginals = {v: 0.0 for v in G.nodes}
    total_prob = 0.0

    for bits in itertools.product([0, 1], repeat=len(non_roots)):
        assignment = dict(root_assignment)
        assignment.update(dict(zip(non_roots, bits)))

        # joint probability under this assignment (roots are fixed)
        prob = 1.0
        for node in topo:
            parents, table = cpts[node]
            p1 = table[tuple(assignment[p] for p in parents)] if parents else table[()]
            val = assignment[node]
            prob *= p1 if val == 1 else (1 - p1)

        total_prob += prob
        for node in G.nodes:
            if assignment[node] == 1:
                marginals[node] += prob

    # normalize to make it conditional on root_assignment
    for node in marginals:
        marginals[node] /= total_prob

    return marginals


def compute_all_marginals(G, cpts):
    roots = [v for v in G.nodes if G.in_degree(v) == 0]
    all_marginals = {}

    # rootノードの全パターンを列挙 (0/1 の組み合わせ)
    for bits in itertools.product([0, 1], repeat=len(roots)):
        root_assignment = dict(zip(roots, bits))
        marginals = compute_conditional_marginals(G, cpts, root_assignment)
        all_marginals[tuple(bits)] = marginals

    return all_marginals


def make_random_dag(num_nodes: int, num_edges: int, seed: int = 0) -> nx.DiGraph:
    rng = random.Random(seed)
    nodes = list(range(num_nodes))
    G = nx.DiGraph()
    G.add_nodes_from(nodes)
    possible_edges = [(u, v) for i, u in enumerate(nodes) for v in nodes[i + 1 :]]
    G.add_edges_from(rng.sample(possible_edges, num_edges))
    assert nx.is_directed_acyclic_graph(G)
    return G


def make_cpts(G: nx.DiGraph, seed: int = 0, low: float = 0.15, high: float = 0.85):
    rng = random.Random(seed)
    cpts = {}
    for node in G.nodes:
        parents = list(G.predecessors(node))
        if not parents:  # root ノードは固定 0.5
            table = {(): 0.5}
        else:
            table = {bits: low + (high - low) * rng.random() for bits in itertools.product([0, 1], repeat=len(parents))}
        cpts[node] = (parents, table)
    return cpts


"""
def make_cpts(G: nx.DiGraph, seed: int = 0):
    rng = random.Random(seed)
    cpts = {}
    for node in G.nodes:
        parents = list(G.predecessors(node))
        table = {bits: rng.betavariate(1, 1) for bits in itertools.product([0, 1], repeat=len(parents))}
        cpts[node] = (parents, table)
    return cpts
"""


def ancestral_sample(G: nx.DiGraph, cpts, rng: random.Random) -> Dict[int, int]:
    sample = {}
    for node in nx.topological_sort(G):
        parents, table = cpts[node]
        p1 = table[tuple(sample[p] for p in parents)]
        sample[node] = 1 if rng.random() < p1 else 0
    return sample


def deterministic_cpts(G: nx.DiGraph, seed: int = 0):
    rng = random.Random(seed)
    cpts = {}

    for node in G.nodes:
        parents = list(G.predecessors(node))
        arity = len(parents)
        flip = tuple(rng.randint(0, 1) for _ in range(arity))

        if arity == 0:
            table = {(): 0.5}
        else:
            table = {
                bits: sum(b ^ f for b, f in zip(bits, flip)) % 2 for bits in itertools.product([0, 1], repeat=arity)
            }

        cpts[node] = (parents, table)

    return cpts


def ancestral_sample_det(G: nx.DiGraph, cpts, rng: random.Random) -> Dict[int, int]:
    sample = {}
    for node in nx.topological_sort(G):
        parents, table = cpts[node]
        if len(parents) == 0:
            sample[node] = 1 if rng.random() < 0.5 else 0
        else:
            sample[node] = table[tuple(sample[p] for p in parents)]
    return sample


class BayesNetOnlineDataset(IterableDataset):
    def __init__(self, config, deterministic=False, split: str = "train", seed: int = 10, chain: bool = False):
        super().__init__()
        self.num_nodes = config["num_nodes"]
        self.num_edges = config["num_edges"]
        self.max_len = config["max_length"]
        self.ignore_index = config["ignore_index"]
        self.mode = split

        self.G = make_random_dag(self.num_nodes, self.num_edges, seed)
        # print("Edges:", self.G.edges())
        if deterministic:
            self.cpts = deterministic_cpts(self.G, seed)
            self._sample_fn = ancestral_sample_det
        else:
            self.cpts = make_cpts(self.G, seed)
            self._sample_fn = ancestral_sample

        self.topo = list(nx.topological_sort(self.G))

        self.roots = [v for v in self.G.nodes if self.G.in_degree(v) == 0]
        self.non_root_indices = [i for i, v in enumerate(self.topo) if v not in self.roots]
        self.query_node = self.topo[-1]
        self.chain = chain

        self.all_marginals = compute_all_marginals(self.G, self.cpts)
        self._build_vocab()
        # print(self.cpts)
        # print(self.all_marginals)
        self.layers = compute_strict_layers(self.G)
        # print(self.layers)
        self.max_width = max(len(layer) for layer in self.layers)
        self.depth = len(self.layers)
        # print(self.depth, self.max_width)

    def _build_vocab(self) -> None:
        self.tok2id: Dict[str, int] = {}
        add = lambda tok: self.tok2id.setdefault(tok, len(self.tok2id))

        add("<pad>")
        add("<mask>")
        add("<eos>")
        add("0")
        add("1")
        for i in range(self.num_nodes):
            add(f"{i}=")

        self.pad_id = self.tok2id["<pad>"]
        # self.mask_id = self.tok2id["<mask>"]

    def _encode_tokens(self, toks: List[str]) -> List[int]:
        return [self.tok2id[t] for t in toks]

    def __iter__(self):
        rng = random.Random(torch.initial_seed() % (2**32))
        while True:
            assign = self._sample_fn(self.G, self.cpts, rng)
            if self.mode == "train":
                if self.chain:
                    tokens, labels = [], []

                    for node in self.topo:
                        var_tok = f"{node}="
                        val_tok = str(assign[node])
                        tokens.extend([var_tok, val_tok])
                        # labels.extend([self.tok2id[val_tok], self.ignore_index])
                else:
                    # select layer index
                    layer_idx = rng.randint(1, len(self.layers) - 1)
                    input_layer = self.layers[layer_idx - 1]
                    target_layer = self.layers[layer_idx]

                    tokens = []
                    for node in input_layer:
                        tokens.extend([f"{node}=", str(assign[node])])
                    last_node = input_layer[-1]
                    while len(tokens) < 2 * self.max_width:
                        tokens.extend([f"{last_node}=", str(assign[last_node])])

                    labels = []
                    for node in target_layer:
                        labels.extend([f"{node}=", str(assign[node])])
                    last_node = target_layer[-1]
                    while len(labels) < 2 * self.max_width:
                        labels.extend([f"{last_node}=", str(assign[last_node])])
                    labels = [self.tok2id[t] for t in labels]
            else:
                # if self.chain:
                tokens, labels = [], []
                for node in self.topo:
                    var_tok = f"{node}="
                    if node in self.roots:
                        tokens.extend([var_tok, str(assign[node])])
                        labels.extend([self.ignore_index, self.ignore_index])
                if not self.chain:
                    input_layer = self.layers[0]
                    tokens = []
                    for node in input_layer:
                        tokens.extend([f"{node}=", str(assign[node])])
                    last_node = input_layer[-1]
                    while len(tokens) < 2 * self.max_width:
                        tokens.extend([f"{last_node}=", str(assign[last_node])])

            input_ids = [self.tok2id[t] for t in tokens]
            if self.mode == "train":
                if self.chain:
                    labels = input_ids[1:] + [self.tok2id["<eos>"]]

                yield torch.tensor(input_ids, dtype=torch.long), torch.tensor(labels, dtype=torch.long)
            else:
                last_node = self.topo[-1]  # とりあえずlast_nodeで固定
                gt_marginal = self.all_marginals[tuple(assign[r] for r in self.roots)][last_node]
                yield torch.tensor(input_ids, dtype=torch.long), torch.tensor([gt_marginal], dtype=torch.float)


class BayesNetTask(GeneralizationTask):
    def __init__(self, num_nodes=16):
        super().__init__()
        # num_nodes = 12
        config = {
            "name": "bayes_net",
            "description": "Ancestor sampling in Bayesian networks.",
            "data_dir": "data/bayes_net",
            "num_nodes": num_nodes,
            "num_edges": num_nodes * 2,
            "max_length": num_nodes * 2,
            "ignore_index": -100,
        }
        config["vocab_size"] = num_nodes + 5
        self.config = config

    def pointwise_loss_fn(self, output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        loss = F.cross_entropy(
            output.view(-1, output.size(-1)), target.view(-1), ignore_index=self.config["ignore_index"]
        )
        return loss

    def accuracy_fn(self, output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        pass  # not used


if __name__ == "__main__":
    # Example usage
    task = BayesNetTask()
    dataset = BayesNetOnlineDataset(task.config, deterministic=True, split="train", chain=False, seed=10)
    for i, (input_ids, label) in enumerate(dataset):
        if i >= 10:
            break
        print(f"Sample {i+1}: Input IDs: {input_ids.tolist()}, Labels: {label.tolist()}")
